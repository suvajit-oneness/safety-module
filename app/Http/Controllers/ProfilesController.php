...
* - getSeperationKey()
* Classes list:
* - ProfilesController extends Controller
*/
...
class ProfilesController extends Controller
{
    ...
    public function show($username)
    {
        ...
        $currentTheme = Theme::find($user
            ->profile
            ->theme_id);

        $data         = ['user' => $user, 'currentTheme' => $currentTheme, ];
    ...
    public function edit($username)
    {
        ...
        $themes       = Theme::where('status', 1)->orderBy('name', 'asc')
            ->get();
        ...
        $data         = ['user' => $user, 'themes' => $themes, 'currentTheme' => $currentTheme,
    ...
    public function update(UpdateUserProfile $request, $username)
    {
        ...
        return redirect('profile/' . $user->name . '/edit')
            ->with('success', trans('profile.updateSuccess'));
    }
    ...
    public function updateUserAccount(Request $request, $id)
    {
        ...
        {
            $usernameRules   = ['name'                 => 'required|max:255|unique:users', ];
        }
        else
        {
        ...
        if ($emailCheck)
        {
            $emailRules      = ['email'                 => 'email|max:255|unique:users', ];
        }
        else
        {
            $emailRules      = ['email'                 => 'email|max:255', ];
        }
        $additionalRules = ['first_name'                 => 'nullable|string|max:255', 'last_name'                 => 'nullable|string|max:255', ];
        ...
        return redirect('profile/' . $user->name . '/edit')
            ->with('success', trans('profile.updateAccountSuccess'));
    }
    ...
    public function updateUserPassword(UpdateUserPasswordRequest $request, $id)
    {
        ...
        return redirect('profile/' . $user->name . '/edit')
            ->with('success', trans('profile.updatePWSuccess'));
    }
